...
type ExplainOptions = { logger: (message?: any, ...optionalParams: any[]) => any };
export const enableExplain = (options?: ExplainOptions) => {
    const logger = options?.logger ?? console.log;

    const notExplainable = ['CREATE', 'ALTER', 'DROP', 'SAVEPOINT', 'COMMENT', 'ROLLBACK', 'BEGIN', 'RELEASE', 'COMMIT', 'SHOW'];
    ...
    Client.prototype.query = async function (this: Client, text: any, values: any, cb: any) {
        ...
        const printDebug = () => {
            ...
            logger(chalk.yellow(typeof text === 'object' ? sqlFormat.format(logQuery) : dedent(logQuery)));
            if (logValues) {
                logger(chalk.gray('\nVariables:'));
